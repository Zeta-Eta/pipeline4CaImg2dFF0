function [DIYparams, shifts, templateUpdating, options] = ...
    normcorre_batch_nonRigid_4GPU(Y, options, template, SPMDcoreNum, DIYparamsON, clearGPU, iterNum, saveON, plotON)
...
%%
tID = NaN(bin_width, ceil(options.mem_batch_size/bin_width));
tID(1:options.mem_batch_size) = 1;
tID = tID.';
tIDstt2end = cumsum(sum(~isnan(tID), 1));
tIDstt2end = [1 + [0, tIDstt2end(1:end - 1)]; tIDstt2end]';
patchNum = T/options.mem_batch_size;
tempID = (0:patchNum - 1)*options.mem_batch_size;

tID = cell(SPMDcoreNum, ceil(patchNum/SPMDcoreNum));
tID(1:patchNum) = {ones(1, bin_width)};
tID(patchNum + 1:end) = {NaN(1, bin_width)};
tID = cell2mat(tID);
tID = tID';

tIDstt = tID;
tIDend = tID;
tIDstt(tID == 1) = tIDstt2end(:, 1) + tempID;
tIDend(tID == 1) = tIDstt2end(:, 2) + tempID;
tIDdiff = tIDend - tIDstt + 1;
tIDdiffSum = max(tIDend, [], 1, 'omitnan') - tIDstt(1, :) + 1;
patchID = tID;
patchID(tID == 1) = repmat(1:patchNum, bin_width, 1);
patchNum4core = [0, max(patchID, [], 1, 'omitnan')];
patchID4core = patchID - patchNum4core(1:end - 1);
patchNum4core = diff(patchNum4core);
...
for iter = 1:iterNum
    ...
    spmd(SPMDcoreNum + 1)
        switch labindex
            ...
            otherwise
                ...
                templateUpdating = zeros([sizY(1:nd), patchNum4core(labindex)]);
                for j = 1:tIDnum(labindex)
                    tSTT = tIDstt(j, labindex);
                    tEND = tIDend(j, labindex);
                    nf = tIDdiff(j, labindex);
                    switch filetype
                        case 'tif'
                    ...
                    if iter == iterNum
                        [shifts{j}, Mf] = register_frame_Rigid_4GPU(Ytm, fftTemp_conj(:, :, patchID(j, labindex)), nf, data_type, ...
                        ...
                        [~, Mf] = register_frame_Rigid_4GPU(Ytm, fftTemp_conj(:, :, patchID(j, labindex)), nf, data_type, ...
                            us_fac, pixNum, pixDim, pixNum2, pixDim2, Nr, Nc, Nr2, Nc2, padID, ...
                            dftNum, dftNum2, dftDim, dftShift, kernR, kernC, interpGridR, interpGridC);
                    end
                    templateUpdating(:, :, patchID4core(j, labindex)) = templateUpdating(:, :, patchID4core(j, labindex)) + sum(Mf, nd + 1);
    ...
    templateUpdating = cat(nd + 1, templateUpdating{1:SPMDcoreNum})./options.mem_batch_size;

    if iter == iterNum
        [shifts2, ~] = register_frame_Rigid_4GPU_makima(templateUpdating, fftTemp_conj, patchNum, data_type, ...
            us_fac, pixNum, pixDim, pixNum2, pixDim2, Nr, Nc, Nr2, Nc2, padID, ...
            dftNum, dftNum2, dftDim, dftShift, kernR, kernC, interpGridR, interpGridC);
    else
        [~, templateUpdating] = register_frame_Rigid_4GPU_makima(templateUpdating, fftTemp_conj, patchNum, data_type, ...
            us_fac, pixNum, pixDim, pixNum2, pixDim2, Nr, Nc, Nr2, Nc2, padID, ...
            dftNum, dftNum2, dftDim, dftShift, kernR, kernC, interpGridR, interpGridC);
    end

    if plotON
        movieTEMP = implay(permute(templateUpdating, [2, 1, 3]), 10);
        movieTEMP.Parent.Position = ...
            [(30 + options.pixDim(1))*2 + (60 + options.pixDim(1))*(iter - 1), basePixSize - options.pixDim(2) - 134, ...
            options.pixDim + [0, 24]].*scaleFactor;
    end
end
toc
