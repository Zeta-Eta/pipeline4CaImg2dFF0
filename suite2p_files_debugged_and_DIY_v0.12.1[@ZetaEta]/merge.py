...
def merge_activity_masks(parent):
    ...
    Fneu = np.zeros((0,parent.Fneu.shape[1]), np.float32)
    dFF0scld = np.zeros((0,parent.dFF0scld.shape[1]), np.float32)
    if parent.hasred:
        F_chan2    = np.zeros((0,parent.Fcell.shape[1]), np.float32)
        Fneu_chan2 = np.zeros((0,parent.Fneu.shape[1]), np.float32)
    ...
    for n in merged_cells:
        ...
        dFF0scld = np.append(dFF0scld, parent.dFF0scld[n,:][np.newaxis,:], axis=0)
        if parent.hasred:
    ...
    _, goodi, ind = np.unique(ipix, return_index=True, return_inverse=True, axis=0)
    ypix = ypix[goodi]
    xpix = xpix[goodi]
    lam2 = np.zeros(len(goodi), np.float32)
    for i in range(len(goodi)):
        lam2[i] = lam[ind == i].sum()
    lam = lam2
    ...
    parent.dFF0scld = np.concatenate((parent.dFF0scld, dFF0scld[np.newaxis,:]), axis=0)
    if parent.hasred:
    ...
    parent.stat[-1]["yext"] = yext
    parent.stat[-1]["xext"] = xext
    # * add colors *
