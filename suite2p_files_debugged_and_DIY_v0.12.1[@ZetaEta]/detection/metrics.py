...
def match_func_anat(stat_func, stat_anat, Ly, Lx, threshold=0.5):
    ...
    for i,sf in enumerate(stat_func):
        ...
        inds = (slice(max(0, ymed-ly), min(ymed+ly, Ly)),
                slice(max(0, xmed-ly), min(xmed+ly, Lx)))
        ...
        for j, sa in enumerate(stat_anat):
            ...
            if (np.logical_and(ypix_a > inds[0].start, ypix_a < inds[0].stop).sum()>0 and
                np.logical_and(xpix_a > inds[1].start, xpix_a < inds[1].stop).sum()>0):
                lam_a = sa['lam'].copy()
...
def extend_anatomical(img_anat, masks_anat, mov=None, ops=None, reg_file=None):
    if mov is None:
        ...
        t0 = time.time()
        with BinaryFile(read_filename=reg_file, Ly=ops['Ly'], Lx=ops['Lx']) as f:
            mov = f.bin_movie(
                                bin_size=bin_size,
    ...
    if ops is not None:
        # process movie
        mov = pca_denoise(mov, [ops['block_size'][0]//2, ops['block_size'][1]//2], ops['PCAdenoiseThres'])
        ...
        mov = neuropil_subtraction(mov=mov / sdmov, filter_size=ops['spatial_hp_detect'])  # subtract low-pass filtered movie
    else:
        ops = {'yrange': [0, Lyc], 'xrange': [0, Lxc]}
        sdmov = np.ones(mov.shape[1:])

    redimg = img_anat[ops['yrange'][0] : ops['yrange'][-1], ops['xrange'][0] : ops['xrange'][-1]]
    redmasks = masks_anat[ops['yrange'][0] : ops['yrange'][-1], ops['xrange'][0] : ops['xrange'][-1]]
    ly = 10
    stat_anat = []
    iorig = []
    for i in range(masks_anat.max()):
        ...
        nb_components, output, stats, centroids = cv2.connectedComponentsWithStats((cc_mask).astype(np.uint8), 
                                                                                connectivity=4)
        npix = stats[1:,-1]
        if (npix>15).sum()==0:
            continue  
