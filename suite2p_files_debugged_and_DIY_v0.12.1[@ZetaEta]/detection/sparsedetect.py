...
def sparsery(mov: np.ndarray, high_pass: int, neuropil_high_pass: int, batch_size: int, spatial_scale: int, threshold_scaling,
             max_iterations: int, percentile=0, splitThres=1.25) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    vmultiplier = max(1, mov.shape[0] / 3600)
    ...
    for tj in range(max_iterations):
        ...
        # check if peak is larger than threshold * max(1,nbinned/3600)
        v_max[tj] = v0max.max()
        ...
        if percentile > 0:
            threshold = np.percentile(tproj, percentile)
        else:
            threshold = Th2
