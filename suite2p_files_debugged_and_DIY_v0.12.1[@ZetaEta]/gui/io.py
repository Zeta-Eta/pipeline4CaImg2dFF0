...
def load_files(name):
    ...
    if stat is not None:
        ...
        try:
            ...
            goodfolder = False
        try:
            dFF0scld = np.load(basename + "/dFF0scld.npy")
        except (ValueError, OSError, RuntimeError, TypeError, NameError):
            print("there are no scaled dFF0 traces in this folder "
                  "(dFF0scld.npy)")
            dFF0scld = Fcell
        try:
    ...
    if goodfolder:
        return stat, ops, Fcell, Fneu, Spks, dFF0scld, iscell, probcell, redcell, probredcell, hasred
    else:
...
def load_to_GUI(parent, basename, procs):
    stat, ops, Fcell, Fneu, Spks, dFF0scld, iscell, probcell, redcell, probredcell, hasred = procs
...
def save_merge(parent):
    ...
    np.save(os.path.join(parent.basename, 'dFF0scld.npy'), parent.dFF0scld)
    iscell =  np.concatenate((parent.iscell[:,np.newaxis],
                              parent.probcell[:,np.newaxis]), axis=1)
