...
def init_views(parent):
    ...
    for k in range(7):
        if k==2:
            ...
            mimg0 = np.percentile(mimg,0)
            mimg100 = np.percentile(mimg,100)
            mimg = (mimg - mimg0) / (mimg100 - mimg0)
            ...
            if 'Vcorr' in parent.ops:
                ...
                mimg0 = np.percentile(vcorr,0)
                mimg100 = np.percentile(vcorr,100)
                vcorr = (vcorr - mimg0) / (mimg100 - mimg0)
                mimg = mimg0 * np.ones((parent.Ly, parent.Lx),np.float32)
            ...
            if 'max_proj' in parent.ops:
                ...
                mimg0 = np.percentile(mproj,0)
                mimg100 = np.percentile(mproj,100)
                mproj = (mproj - mimg0) / (mimg100 - mimg0)
                mimg = np.zeros((parent.Ly, parent.Lx),np.float32)
                try:
                    mimg[parent.ops['yrange'][0]:parent.ops['yrange'][1],
