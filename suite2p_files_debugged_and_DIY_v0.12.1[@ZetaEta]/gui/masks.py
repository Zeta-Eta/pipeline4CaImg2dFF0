...
def init_masks(parent):
    ...
    parent.rois['LamMean'] = LamAll[LamAll>0].mean()
...
def draw_masks(parent): #ops, stat, ops_plot, iscell, ichosen):
    ...
    if view==0:
        ...
        for n in parent.imerge:
            ...
            # M[wplot] = make_chosen_bndry(M[wplot], ycirc, xcirc, col)

            yext = parent.stat[n]['yext']
            xext = parent.stat[n]['xext']
            id = ((xext>=parent.ops['xrange'][0]) & (xext<parent.ops['xrange'][1]) & 
                  (yext>=parent.ops['yrange'][0]) & (yext<parent.ops['yrange'][1]))
            col = parent.colors['cols'][color,n]
            M[wplot] = make_chosen_bndry(M[wplot], yext[id], xext[id], col)
...
def make_chosen_bndry(M0, yB, xB, col):
    M0[yB,xB,:3] = col
    M0[yB,xB,3]  = 255
    return M0
