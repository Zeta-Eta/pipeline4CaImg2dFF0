...
def plot_trace(parent):
    ...
    if len(parent.imerge)==1:
        ...
        fneu = parent.Fneu[n,:]
        sp = parent.Spks[n,:]
        dFF0scld = parent.dFF0scld[n,:]
        if np.ptp(fneu)==0:
            fmax = np.maximum(f.max(), dFF0scld.max())
            fmin = np.minimum(f.min(), dFF0scld.min())
        else:
            fmax = np.maximum(np.maximum(f.max(), dFF0scld.max()), fneu.max())
            fmin = np.minimum(np.minimum(f.min(), dFF0scld.min()), fneu.min())
        #sp from 0 to fmax
        sp = sp*fmax/sp.max()
        #agus
        fmin = np.minimum(fmin, 0)
        #sp += fmin*0.95
        if parent.deconvOn:
            parent.p3.plot(parent.trange,sp,pen=(255,255,255,150))
        if parent.tracesOn:
            parent.p3.plot(parent.trange,f,pen='c')
        if parent.neuropilOn:
            parent.p3.plot(parent.trange,fneu,pen='r')
        if parent.dFF0scldOn:
            parent.p3.plot(parent.trange,dFF0scld,pen='g')
        parent.fmin= fmin
        parent.fmax=fmax
        ax.setTicks(None)
    else:
...
def make_buttons(parent, b0):
    ...
    parent.comboBox.addItem("scaled dFF0")
    parent.activityMode = 0
    ...
    parent.l0.addWidget(parent.checkBoxd, b0, 9, 1, 2)
    # neuropil CHECKBOX
    parent.l0.setVerticalSpacing(4)
    ...
    parent.l0.addWidget(parent.checkBoxn, b0, 7, 1, 2)
    # traces CHECKBOX
    parent.l0.setVerticalSpacing(4)
    ...
    parent.l0.addWidget(parent.checkBoxt, b0, 5, 1, 2)
    # dFF0scld CHECKBOX
    parent.l0.setVerticalSpacing(4)
    parent.checkBoxs = QCheckBox("scaled dFF0 [C]")
    parent.checkBoxs.setStyleSheet("color: green;")
    parent.checkBoxs.toggled.connect(lambda: dFF0scld_on(parent))
    parent.dFF0scldOn = False
    parent.l0.addWidget(parent.checkBoxs, b0, 3, 1, 2)
    return b0
...
def dFF0scld_on(parent):
    state = parent.checkBoxs.isChecked()
    if parent.loaded:
        if state:
            parent.dFF0scldOn = True
        else:
            parent.dFF0scldOn = False
        plot_trace(parent)
        parent.win.show()
